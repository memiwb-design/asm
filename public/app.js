...
// Configuration - API endpoint (backend server)
const CONFIG = {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Real API Submit - sends multipart/form-data to the backend
    btnSubmit.addEventListener('click', async () => {
        ...
        btnSubmit.innerHTML = `<div class="loading-spinner"></div><span>Invio in corso...</span>`;

        try {
            const formData = new FormData();
            formData.append('category', state.selectedCategory);
            formData.append('notes', notes || '');
            formData.append('latitude', state.gps.lat || '');
            formData.append('longitude', state.gps.lng || '');
            formData.append('address', state.gps.address || '');

            // Attach actual file objects from the input (preserving original binary)
            ...
            if (fileInput && fileInput.files.length > 0) {
                ...
                // Fallback: convert base64 previews back to blobs
                state.photos.forEach((photoDataUrl, i) => {
                    try {
                        const blob = dataURItoBlob(photoDataUrl);
                        formData.append('photos', blob, `foto_${i + 1}.jpg`);
                    } catch (e) { console.warn('Blob conversion skipped:', e); }
                });
            }
            ...
            if (response.ok && data.success) {
                ...
                document.getElementById('summary-photos').textContent = data.report.photos_count || state.photos.length;
                ...
                renderPhotos();
                showToast(`Segnalazione #${String(data.report.id).padStart(4,'0')} inviata con successo!`, "success");
            } else {
                const errMsg = data.details ? data.details.join(' ') : (data.error || 'Errore invio.');
                showToast(`Errore: ${errMsg}`, "warning");
            }

        } catch (err) {
            console.error('API Error:', err);
            // Network error fallback: open mailto client
            showToast("Connessione assente. Apertura e-mail...", "warning");
